...
const TextInput = ({ type = 'text', autoFocus, name, label, handleChange, value, gutterBottom, startIcon, textArea }) => {
  ...
  return (
    <div className={`relative w-full ${gutterBottom ? 'mb-4' : ''}`}>
      ...
      {textArea ? <textarea name={name} id={name} onChange={handleChange} value={value} className={`${classes} resize-none`} rows={8} /> : <input autoFocus={autoFocus} type={inputType} name={name} id={name} onChange={handleChange} value={value} className={classes} />}
